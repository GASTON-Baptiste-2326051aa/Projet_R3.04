...
public class SetIllness implements Set<Illness> {

    private Illness[] illnesses;

    public SetIllness() {
        this.illnesses = new Illness[]{};
    }

    public SetIllness(Illness[] illnesses) {
     ...
     */
    @Override
    public Iterator<Illness> iterator() {
        return Arrays.stream(illnesses).iterator();
    }

    private void refreshSet() {
        for (int i = 0; i < illnesses.length; i++) {
            if (illnesses[i] == null) {
                for (int j = i; j < illnesses.length - 1; j++) {
                    illnesses[j] = illnesses[j + 1];
                }
                Illness[] tmp = new Illness[illnesses.length - 1];
                System.arraycopy(illnesses, 0, tmp, 0, illnesses.length - 1);
                illnesses = tmp;
            }
        }
    }
     ...
     *
     * @param illness element to be added to this set
     ...
     */
    @Override
    public boolean add(Illness illness) {
        ...
        Illness[] tmp = new Illness[illnesses.length + 1];
        System.arraycopy(illnesses, 0, tmp, 0, illnesses.length);
        tmp[illnesses.length] = illness;
        illnesses = tmp;
        return true;
    }
     ...
     */
    @Override
    public boolean addAll(Collection<? extends Illness> c) {
        boolean changed = false;
        for (Illness illness : c) {
            if (add(illness)) {
                changed = true;
            }
        }
